...
| **Acesso** | ZIP/XML BVBG-086 (b3.com.br) com fallback HTML (www2.bmf.com.br) + CSV API (arquivos.b3.com.br) |
| **Formato** | ZIP nested (XML streaming via lxml.etree.iterparse) + HTML fallback (BeautifulSoup) + CSV (`;` separator) |
...
A B3 publica diariamente os ajustes (settlement prices) de todos os contratos futuros negociados. A fonte primaria e o arquivo BVBG-086 (ZIP nested contendo snapshots XML) disponivel em `b3.com.br/pesquisapregao/download`. O ZIP contem snapshots intradiarios; o agrobr usa o ultimo (snapshot 03, definitivo). Parsing via `lxml.etree.iterparse` (streaming, sem carregar XML inteiro em memoria).

Fallback automatico para a pagina HTML legada em `www2.bmf.com.br` (tabela `tblDadosAjustes`) em caso de falha no ZIP. Em ambos os caminhos, filtra apenas os 7 contratos agricolas e produz o mesmo schema de saida.
...
- Ajustes: fonte primaria (BVBG-086 ZIP) pode ficar indisponivel — fallback automatico para HTML legado
...
- [Ajustes Diarios (legado)](https://www2.bmf.com.br/pages/portal/bmfbovespa/boletim1/Ajustes1.asp)
