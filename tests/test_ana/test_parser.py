...
from agrobr.ana.parser import PARSER_VERSION, parse_layer_tabular
...
class TestParseTabularPivos:
    def test_tabular_pivos(self):
        arcgis = _geojson_to_arcgis_json(_geojson_bytes())
        df = parse_layer_tabular([arcgis], layer_key="pivos_irrigacao")
    ...
    def test_empty_pages(self):
        df = parse_layer_tabular([], layer_key="pivos_irrigacao")
...
class TestParseGeojsonPivos:
    def _parse(self):
        from agrobr.ana.parser import parse_layer_geojson

        data = _geojson_bytes()
        return parse_layer_geojson([data], layer_key="pivos_irrigacao")

    def test_geojson_pivos(self):
        gdf = self._parse()
        assert len(gdf) == 5
    ...
    def test_empty_pages(self):
        ...
        gdf = parse_layer_geojson([], layer_key="pivos_irrigacao")
