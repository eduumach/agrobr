...
from agrobr.rio_verde.parser import PARSER_VERSION, _extract_records, _records_to_df

GOLDEN_DIR = Path(__file__).resolve().parent.parent / "golden_data" / "rio_verde"


def _parse_pages(pages: list[str], safra: str) -> pd.DataFrame:
...
class TestParseEnsaioSoja:
    def test_golden_columns(self, golden_data):
        df = _parse_pages(golden_data["pages"], golden_data["safra"])
        expected = [
            "safra",
            "empresa",
            "cultivar",
    ...
    def test_empty_pages_raises(self):
        with pytest.raises(ParseError):
            _parse_pages([], "2025/2026")

    def test_no_data_pages_raises(self):
        with pytest.raises(ParseError):
            _parse_pages(["no data here"], "2025/2026")
...
class TestSectionDetectionResilience:
    def test_case_insensitive_entry(self):
        ...
        df = _parse_pages(pages, "2025/2026")
        assert len(df) == 1
