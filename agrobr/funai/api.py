...
def _validate_uf(uf: str | None) -> str | None:
    ...
    if not _UF_RE.match(uf_upper):
        raise ValueError(f"UF invalida: {uf!r} (esperado 2 letras maiusculas, ex: 'MT')")
    return uf_upper
...
async def terras_indigenas_geo(
    ...
    **kwargs: Any,  # noqa: ARG001
    ...
    t0 = time.monotonic()
    geojson_bytes, source_url = await client.fetch_terras_indigenas_geo(uf=uf, fase=fase, bbox=bbox)
    fetch_ms = int((time.monotonic() - t0) * 1000)
