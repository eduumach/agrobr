...
- **desmatamento** — `deter_geo_stream(bioma=, uf=, data_inicio=, data_fim=, classe=)` itera os alertas DETER com geometria em batches (`AsyncGenerator[GeoDataFrame, None]`), sem acumular tudo em memoria nem esbarrar no teto de `MAX_FEATURES_GEO` da chamada unica de `deter_geo()`. `client.stream_deter_geo()` pagina a WFS via `startIndex`/`count` (WFS 2.0.0) yieldando uma pagina por vez com throttle pos-pagina. Async-only (sem suporte em `agrobr.sync`)
- **mapbiomas_alerta** — `alertas_geo_stream(token=, start_date=, end_date=, sources=, bbox=, limit=, max_pages=)` itera os alertas geoespaciais em batches (`AsyncGenerator[GeoDataFrame, None]`), evitando acumular todas as geometrias WKT em memoria. `client.stream_alertas()` pagina o GraphQL yieldando a `collection` de cada pagina; `fetch_alertas()` agora delega a esse gerador. Dedup de `alert_code` aplicado entre batches. Async-only (sem suporte em `agrobr.sync`)
- **utils.geo** — `stream_wfs_paginated()`: gerador assincrono generico de paginacao WFS (conta via `resultType=hits`, pagina por `startIndex`/`count`) reutilizavel por qualquer fonte GeoServer; `fetch_wfs_paginated()` agora delega a ele e ganhou o parametro `output_format`
